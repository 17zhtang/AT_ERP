...
/***
 * @用例名称:设备管理-设备列表-新增设备-智能用电管控终端
 * @author Linjf
 */
public class H710_0010 extends _H710 implements TestCase{
	public H710_0010(){
		super();
	}
	public H710_0010(WebDriver driver){
		super(driver);
	}
	
	@Override
	public void testcase() throws Exception{
		String sheetname = H710_0010.class.getPackage().getName().split("\\.")[4];
		int index=ExcelOperate.getclassname_rows(this.getClass().getSimpleName(), sheetname);
		
		try{//CaseStepStart
			
			String deviceType="智能用电管控终端";
			String deviceName="管控终端"+PM.sjs6;
			//h710_SBXZ(deviceType,deviceName,"5918",getGuid(),getRandomIp(),"自动化基础数据","7楼","7号","茶水间","设备规格00001","优柏","","1");
			h710_SBXZ(deviceType, deviceName,getGuid(), getRandomIp());
			String sql=new StringBuffer(
					"select deviceName from es_deviceinfo where deviceName=")
			.append("\'").append(deviceName).append("\'").toString();
			
			action.assertReport(sql, deviceName, "用例验证成功", "用例验证失败", sheetname, index);
