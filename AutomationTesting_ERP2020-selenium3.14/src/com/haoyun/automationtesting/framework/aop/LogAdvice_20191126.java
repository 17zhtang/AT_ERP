 ...
 *
 */
public class LogAdvice_20191126 {
	...
	public Object log(ProceedingJoinPoint pjp) throws Throwable {
		log.logWarn("开始的时间"+new SimpleDateFormat("HH-mm-ss-SSSS").format(new Date()));
		...
		// System.out.println("一共有多少给sheet页："+wb.getNumberOfSheets());
        	int gnos=wb.getNumberOfSheets();
        	//
		for (int sheetindex = 1; sheetindex < gnos; sheetindex++) {// 获取sheet页个数
			String sheetname = wb.getSheetName(sheetindex);
			 //System.out.println(gnos+"sheetname::"+sheetindex+":"+sheetname);
			...
			if (!sheetname.trim().isEmpty()) {
				...
				for (int i = 5; i <= rowindex + 1; i++) {
					...
					String getsn=ExcelOperate.getexcel(i, 5, sheetname);
					//log.logWarn("开始的时间2222"+new SimpleDateFormat("HH-mm-ss-SSSS").format(new Date()));
					if (getsn.equals("1")) {// 执行excel中状态为1的用例
						//System.out.println(i+"次"+ExcelOperate.getexcel(i, 1, sheetname));
						...
						if (ExcelOperate.getexcel(i, 1, sheetname).equals(
							...
							log.logInfo("开始执行用例："+ method);
							result = pjp.proceed();
							
							log.logInfo("执行完成'"+ method+"'所用时间:"+
				...
				//log.logWarn("开始的时间3333"+new SimpleDateFormat("HH-mm-ss-SSSS").format(new Date()));
			}
        ...
        log.logWarn(result+"最后的时间"+new SimpleDateFormat("HH-mm-ss-SSSS").format(new Date()));
		return result;
