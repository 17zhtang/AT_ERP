...
public class LogAdvice {
	...
	public Object log(ProceedingJoinPoint pjp) throws Throwable {
		...
		try {
			...
			String calssname =classcase[classcase.length - 1];
			String stname =classcase[classcase.length - 2];
			// ExcelOperate.getexcel(i, 1, stname);
			// System.out.println(stname+"sheet页："+calssname);
			if (!stname.trim().isEmpty()) {

				Sheet st = wb.getSheet(stname);// 选择执行测试案例的sheet页
				int row = st.getLastRowNum();
				//System.out.println(stname+"sheet页的行数："+row);
				int rowindex = 0;
				...
				for (int i = 5; i <= rowindex + 1; i++) {
					...
					if (ExcelOperate.getexcel(i, 1, stname).equals(calssname)) {
						if (ExcelOperate.getexcel(i, 5, stname).equals("1")) {
							log.logInfo("开始执行用例：" + method);
							//PM.return_sy();//每条用例之前都要加返回首页的操作。
							result = pjp.proceed();
		...
		//log.logWarn("结束的时间"+new SimpleDateFormat("HH-mm-ss-SSSS").format(new Date()));
		return result;
