...
/***
 * 
 * 功能说明：将自动化录制工具生成的代码经过筛选转换到本框架中。取外部文件的路径是 在config.xml:AutoSeleniumIDECode_pwd
 * 文件名命名规则：
 * 1、前面是模块名称（包名）后面是用例名（类名），中间以“666”隔开
...
public class AutoSeleniumIDECode {

	@Test
	//public static void main(String[] args){
	public static void run() throws Exception {
		try {
			String in = JOptionPane.showInputDialog(null,"请输入要转换的文件名（前面是模块名称（包名）后面是用例名（类名），中间以“666”隔开）:","要转换文件的名称");
			if (in.trim().isEmpty()) {
				...
				if (in.trim().contains("666") && !in.trim().startsWith("666")
					...
					if(((c1>='a'&&c1<='z')||(c1>='A'&&c1<='Z'))&&((c2>='a'&&c2<='z')||(c2>='A'&&c2<='Z'))){
							...
							charOutStream(in);//执行
							System.out.println("用例生成完成，如果是在eclipse中操作，请在操作后刷新代码后才能显示");
							JOptionPane.showMessageDialog(null, "如果是在eclipse中操作，请在操作后刷新代码后才能显示", "用例生成完成",
									JOptionPane.PLAIN_MESSAGE);
							
						
					}else{
						JOptionPane.showMessageDialog(null, "第一位和666后的第一位必须为字母",
								"输入文件名格式不正确", JOptionPane.ERROR_MESSAGE);
					}
	                 
					
					
				} else {
					JOptionPane.showMessageDialog(null, "要包含666",
							"输入文件名格式不正确", JOptionPane.ERROR_MESSAGE);
				}
	...
	public static String filerule(String extfile) throws Exception {
		...
		fileName = file_pwd() + "\\" + extfile + ".java";
		...
		try{
		jianqie1 = jianqie.split("@Test")[1];
		} catch (Exception e) {
			
			System.out.println("文件中没有@Test字样，可能不是需要转换的脚本");
			e.printStackTrace();
		}
...
try{
		...
		for (int i = 0; i < length; i++) {// 对单个动作进行判断写入操作
			...
			if (fileNameNow.trim().startsWith("driver")) {
				sb_all.append("            " + fileNameNow.trim() + "\r\n");
			}
		}
} catch (Exception e) {
	e.printStackTrace();
	...
	public static String file_pwd() throws IOException {//文件存放路径
		String filePath = "";

		return filePath =Config.AutoSeleniumIDEToCode_pwd; //"C:/Users/Administrator/Downloads";
	}
